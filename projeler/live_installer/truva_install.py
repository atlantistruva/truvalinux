...
# Adapted to truva by Kemas Antonius <http://truva.abangadek.com/>
...
def execute_install( device ):
	...
	oscmd = ( '%(exedir)s/truva_install_run.py -g --device=%(device)s &' % vars() )
	os.system( oscmd )
...
class truvaLinuxInstall(SimpleGladeApp):
	...
	def run_install(self):	
		...
		if cbidx > 0:
			...
			self.text.set_text('\nLütfen Truva Çalışan CD\'yi kuracağınız\ndisk bölümünü seçiniz...\n')
			self._buttonfunc = 2
		else:
			self.text.set_text('\n\nKurulumun yapılabilmesi için\n gerekli 2 GB\'lık disk bölümü bulunamadı.\nBu durumda kuruluma devam edilemez!\n')		
